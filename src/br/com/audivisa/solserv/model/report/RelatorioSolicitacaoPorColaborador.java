...
public class RelatorioSolicitacaoPorColaborador {
	...
	public RelatorioSolicitacaoPorColaborador(String idColaborador, String idPrioridade, String idSituacao) {
	...
	public byte[] getDynamicJasperReport() {
		...
		if (this.idColaborador != null) {
			filter += " s.colaborador.id = " + this.idColaborador.toString();
		}
		...
		String title = "Solicitações por Colaborador";
	...
	private DynamicReport buildReport(String title) throws Exception {
		...
		AbstractColumn columnColaborador = ColumnBuilder.getNew()
				.setColumnProperty("colaborador.nome", String.class.getName())	
		...
		DJGroup g1 = gb1.setCriteriaColumn((PropertyColumn) columnColaborador)
				.setGroupLayout(GroupLayout.VALUE_IN_HEADER_WITH_HEADERS)
				.setHeaderHeight(20)
				.build();
		
		drb.addColumn(columnColaborador);
